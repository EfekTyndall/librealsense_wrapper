...
for folder in [subfolder_depth, subfolder_rgb, subfolder_depth_unaligned, subfolder_rgb_unaligned]:
...
subfolder_rgb_unaligned_foundationpose = os.path.join(foundationpose_dir, "rgb_unaligned")

# Create the subfolders for out_foundationpose if they do not exist
for folder in [subfolder_depth_foundationpose, subfolder_rgb_foundationpose, subfolder_depth_unaligned_foundationpose, subfolder_rgb_unaligned_foundationpose]:
    if not os.path.exists(folder):
        os.makedirs(folder)
...
try:
    while True:
        ...
        # Show the elapsed time while recording
        if RecordStream:
            elapsed_time = time.time() - start_time
            cv2.putText(images, f"Recording: {elapsed_time:.2f} s", (50, 50), 
        ...
        # Handle 'c' press to capture a single frame for out_foundationpose
        if key & 0xFF == ord("c"):
            framename = int(round(time.time() * 1000))
